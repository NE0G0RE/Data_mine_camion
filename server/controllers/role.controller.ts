...
import { eq, and, inArray, sql, or, isNull, gte } from 'drizzle-orm';
import { roles, roleFeatures, users, userRoles, features } from '../../shared/schema.js';
import type { Role, NewRole } from '../../shared/schema/role.schema.js';
import type { UserRole } from '../../shared/schema/user-role.schema.js';
import type { RoleFeature } from '../../shared/schema/role-feature.schema.js';
import type { Feature } from '../../shared/schema/feature.schema.js';
...
class RoleController {
    ...
    try {
      ...
      const newRole = await db.transaction(async (tx) => {
        ...
        if (permissions && permissions.length > 0) {
          ...
          await tx.insert(roleFeatures).values(roleFeaturesToInsert as any); // Temporary any to bypass type checking
        }
      ...
      await db.transaction(async (tx) => {
        ...
        await tx.update(roles)
          .set({
            ...
            updatedAt: new Date(),
            dateModification: new Date()
          } as any) // Temporary any to bypass type checking
          .where(eq(roles.id, id));
      ...
      const usersWithRole = await db
        ...
        .where(
          and(
            ...
            sql`${userRoles.dateFin} IS NULL OR ${userRoles.dateFin} >= CURRENT_DATE`
          )
        )
        .limit(1);
  ...
  async getRoleById(req: Request, res: Response) {
    try {
      ...
      const response = {
        ...roleWithFeatures,
        features: (roleWithFeatures.roleFeatures as Array<RoleFeature & { feature: Feature }>).map(rf => ({
          id: rf.featureId,
  ...
  async getUserRoles(req: Request, res: Response) {
    try {
      ...
      const response = (userRoleData as Array<{
        id: string;
        roleId: string;
        role: { name: string };
        dateDebut?: Date | null;
        ...
        actif?: boolean;
        // Add any other fields that might be present in the database
        [key: string]: any;
      }>).map(ur => ({
        id: ur.id,
        roleId: ur.roleId,
    ...
    try {
      ...
      for (const { roleId } of activeRoles) {
        const permission = await featureService.hasPermission(
          roleId,
          featureCode,
          action as 'voir' | 'modifier' | 'supprimer'
        );
