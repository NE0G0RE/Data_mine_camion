...
class DashboardController {
        ...
        const recentFeatures = await db
          ...
          .where(
            and(
              ...
              gte(features.createdAt, new Date(Date.now() - 30 * 24 * 60 * 60 * 1000))
            )
          )
        ...
        const popularFeatures = await db
          ...
          .leftJoin(
            ...
            and(
              ...
              gte(roleFeatures.createdAt, new Date(Date.now() - 30 * 24 * 60 * 60 * 1000)),
              lte(roleFeatures.createdAt, new Date())
            )
          )
          .where(eq(features.isActive, true))
  ...
  async getFeatureUsageStats(req: Request, res: Response) {
    try {
      ...
      const featureUsage = await db
        ...
        .from(features)
        .leftJoin(roleFeatures, and(
          eq(features.id, roleFeatures.featureId),
          gte(roleFeatures.dateAttribution, start),
          lte(roleFeatures.dateAttribution, end)
        ))
   ...
   */
  async getRoleUsageStats(req: Request, res: Response) {
    try {
      const db = await getDb();
  ...
  async getUserActivity(req: Request, res: Response) {
    try {
      ...
      const activityByUser = recentActivity.reduce<Record<string, any>>((acc, activity) => {
        if (!acc[activity.id]) {
          acc[activity.id] = {
            id: activity.id,
