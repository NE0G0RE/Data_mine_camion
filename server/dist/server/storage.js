import { getDb, trucks, filiales, roles, utilisateurs, permissionsUtilisateur } from "./db.js";
...
export class MySQLStorage {
    ...
    async getPermissionsByUtilisateur(utilisateurId) {
        const db = await getDb();
        return await db.select().from(permissionsUtilisateur).where(and(eq(permissionsUtilisateur.utilisateurId, utilisateurId), eq(permissionsUtilisateur.actif, true)));
    }
    ...
    async getTruckByImmatriculation(immatriculation, filialeId) {
        const db = await getDb();
        if (filialeId) {
            const result = await db.select().from(trucks).where(and(eq(trucks.immatriculation, immatriculation), eq(trucks.filialeId, filialeId))).limit(1);
            return result[0];
        }
    ...
    async updateTruck(id, updateData) {
        ...
        const updateValues = {};
        if (updateData.filialeId !== undefined)
            updateValues.filialeId = updateData.filialeId;
        if (updateData.immatriculation !== undefined)
            updateValues.immatriculation = updateData.immatriculation;
        if (updateData.modele !== undefined)
            updateValues.modele = updateData.modele;
        if (updateData.numeroDA !== undefined)
            updateValues.numeroDA = updateData.numeroDA || null;
        if (updateData.dateDA !== undefined)
            updateValues.dateDA = updateData.dateDA ? new Date(updateData.dateDA) : null;
        if (updateData.daValide !== undefined)
            updateValues.daValide = updateData.daValide || null;
        if (updateData.numeroCA !== undefined)
            updateValues.numeroCA = updateData.numeroCA || null;
        if (updateData.dateCA !== undefined)
            updateValues.dateCA = updateData.dateCA ? new Date(updateData.dateCA) : null;
        if (updateData.dateReception !== undefined)
            updateValues.dateReception = updateData.dateReception ? new Date(updateData.dateReception) : null;
        if (updateData.validationReception !== undefined)
            updateValues.validationReception = updateData.validationReception || null;
        if (updateData.installePar !== undefined)
            updateValues.installePar = updateData.installePar || null;
        if (updateData.dateInstallation !== undefined)
            updateValues.dateInstallation = updateData.dateInstallation ? new Date(updateData.dateInstallation) : null;
        if (updateData.parametrageRealise !== undefined)
            updateValues.parametrageRealise = updateData.parametrageRealise || null;
        if (updateData.localisationFonctionnelle !== undefined)
            updateValues.localisationFonctionnelle = updateData.localisationFonctionnelle || null;
        if (updateData.statutConduite !== undefined)
            updateValues.statutConduite = updateData.statutConduite || null;
        if (updateData.telechargementMemoireMasse !== undefined)
            updateValues.telechargementMemoireMasse = updateData.telechargementMemoireMasse || null;
        if (updateData.numeroTruck4U !== undefined)
            updateValues.numeroTruck4U = updateData.numeroTruck4U || null;
        if (updateData.presenceTablette !== undefined)
            updateValues.presenceTablette = updateData.presenceTablette || null;
        if (updateData.typeTablette !== undefined)
            updateValues.typeTablette = updateData.typeTablette || null;
        if (updateData.imei !== undefined)
            updateValues.imei = updateData.imei || null;
        if (updateData.fonctionnelle !== undefined)
            updateValues.fonctionnelle = updateData.fonctionnelle || null;
        if (updateData.compatibilite !== undefined)
            updateValues.compatibilite = updateData.compatibilite || null;
        if (updateData.deliverup !== undefined)
            updateValues.deliverup = updateData.deliverup || null;
        if (updateData.applicationsSpecifiques !== undefined)
            updateValues.applicationsSpecifiques = updateData.applicationsSpecifiques || null;
        if (updateData.raisonsNonInstalle !== undefined)
            updateValues.raisonsNonInstalle = updateData.raisonsNonInstalle || null;
        if (updateData.cameraCabineTelematics !== undefined)
            updateValues.cameraCabineTelematics = updateData.cameraCabineTelematics || null;
        if (updateData.dashcam !== undefined)
            updateValues.dashcam = updateData.dashcam || null;
        if (updateData.numeroPDA !== undefined)
            updateValues.numeroPDA = updateData.numeroPDA || null;
        if (updateData.materielRequis !== undefined)
            updateValues.materielRequis = updateData.materielRequis || null;
        if (updateData.testsOK !== undefined)
            updateValues.testsOK = updateData.testsOK || null;
        if (updateData.champAction !== undefined)
            updateValues.champAction = updateData.champAction || null;
        if (updateData.observations !== undefined)
            updateValues.observations = updateData.observations || null;
    ...
    async searchTrucks(query, filialeId) {
        ...
        if (filialeId) {
            return await db.select().from(trucks).where(and(eq(trucks.filialeId, filialeId), or(like(trucks.immatriculation, searchTerm), like(trucks.modele, searchTerm), like(trucks.imei, searchTerm), like(trucks.numeroTruck4U, searchTerm), like(trucks.numeroPDA, searchTerm))));
        }
        return await db.select().from(trucks).where(or(like(trucks.immatriculation, searchTerm), like(trucks.modele, searchTerm), like(trucks.imei, searchTerm), like(trucks.numeroTruck4U, searchTerm), like(trucks.numeroPDA, searchTerm)));
    }
    async filterTrucksByStatus(status, filialeId) {
        const db = await getDb();
        if (filialeId) {
            return await db.select().from(trucks).where(and(eq(trucks.filialeId, filialeId), or(eq(trucks.daValide, status), eq(trucks.validationReception, status), eq(trucks.parametrageRealise, status), eq(trucks.statutConduite, status), eq(trucks.presenceTablette, status), eq(trucks.fonctionnelle, status), eq(trucks.compatibilite, status), eq(trucks.deliverup, status), eq(trucks.cameraCabineTelematics, status), eq(trucks.dashcam, status), eq(trucks.materielRequis, status), eq(trucks.testsOK, status))));
        }
        return await db.select().from(trucks).where(or(eq(trucks.daValide, status), eq(trucks.validationReception, status), eq(trucks.parametrageRealise, status), eq(trucks.statutConduite, status), eq(trucks.presenceTablette, status), eq(trucks.fonctionnelle, status), eq(trucks.compatibilite, status), eq(trucks.deliverup, status), eq(trucks.cameraCabineTelematics, status), eq(trucks.dashcam, status), eq(trucks.materielRequis, status), eq(trucks.testsOK, status)));
    }
}
