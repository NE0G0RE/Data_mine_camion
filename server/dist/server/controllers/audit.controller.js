...
import { auditLogs, AUDIT_ACTIONS, AUDIT_ENTITY_TYPES } from '../../shared/schema/audit.schema.js';
import { users } from '../../shared/schema/user.schema.js';
    ...
    async getAuditStats(req, res) {
        try {
            ...
            const actionsByType = await db
                ...
                .from(auditLogs)
                .where(and(gte(auditLogs.createdAt, start), lte(auditLogs.createdAt, end)))
            ...
            const actionsByUser = await db
                ...
                userName: sql `CONCAT(${users.prenom}, ' ', ${users.nom})`,
                count: sql `count(*)`,
            })
                .from(auditLogs)
                .leftJoin(users, eq(auditLogs.userId, users.id))
            ...
            const actionsByEntity = await db
                ...
                .from(auditLogs)
                .where(and(gte(auditLogs.createdAt, start), lte(auditLogs.createdAt, end), sql `${auditLogs.entityType} IS NOT NULL`))
                .groupBy(auditLogs.entityType)
                .orderBy(desc(sql `count(*)`));
            // Récupérer l'activité par jour
            const activityByDay = await db
                .select({
                date: sql `DATE(${auditLogs.createdAt})`,
                count: sql `count(*)`,
            })
                .from(auditLogs)
                .where(and(gte(auditLogs.createdAt, start), lte(auditLogs.createdAt, end)))
