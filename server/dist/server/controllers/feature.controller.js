...
export class FeatureController {
    ...
    async updateFeature(req, res, next) {
        try {
            ...
            const updatedFeature = await featureService.updateFeature(id, validation.data);
            if (!updatedFeature) {
                return res.status(404).json({ message: "Fonctionnalité non trouvée" });
            }
    ...
    async updateRolePermission(req, res, next) {
        try {
            ...
            const updatedPermission = await featureService.updateRolePermission(roleId, featureId, permissions, req.user.id // ID de l'utilisateur qui effectue la modification
            );
     ...
     */
    async getRoleFeatures(req, res, next) {
        try {
    ...
    async checkPermission(req, res, next) {
        try {
            ...
            const hasPermission = await featureService.hasPermission(userRole, featureCode, action);
