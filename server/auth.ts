...
export class AuthService {
  // Authentification
  async authenticateUser(email: string, password: string): Promise<Utilisateur | null> {
  ...
  async getUserPermissions(userId: string): Promise<UserPermissions> {
    ...
    const permissions = await db
      ...
      .where(
        and(
          eq(permissionsUtilisateur.utilisateurId, userId),
    ...
    const accessibleFiliales: string[] = [];
  ...
  async canAccessFiliale(userId: string, filialeId: string): Promise<boolean> {
  ...
  async canManageFiliale(userId: string, filialeId: string): Promise<boolean> {
    const permissions = await this.getUserPermissions(userId);
    ...
    const hasResponsableRole = permissions.roles.some(
      r => r.niveau === 2 && r.filialeId === filialeId
    );
  ...
  async authenticateToken(token: string): Promise<Utilisateur | null> {
