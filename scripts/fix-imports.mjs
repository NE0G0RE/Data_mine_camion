...
function fixImports(filePath) {
  ...
  content = content.replace(/from ["']@\/(.*?)["']/g, (match, p1) => {
    const relativePath = path.relative(path.dirname(filePath), path.resolve(projectRoot, 'client/src')).replace(/\\/g, '/');
  ...
  content = content.replace(/from ["']@shared\/(.*?)["']/g, (match, p1) => {
    const relativePath = path.relative(path.dirname(filePath), path.resolve(projectRoot, 'shared')).replace(/\\/g, '/');
    return `from "${relativePath}/${p1}"`;
  });
